...
def getImageAreas(imgFile):
    # TODO: run nude.py to detect the skin regions before runing facedetect
    featureData = dict()
...
#        calcAreas = json.load(calc_fd)
    ...
    with open('allFeaturesData.csv', 'wb') as csv_fd:
        ...
        for key in trainingData.keys():
            ...
            print  forehead, eyeSidesLeft, eyeSidesRight, lipSidesLeft, lipSidesRight , chin
...
def runC45FromFileData():
    c45Input = dict()
    c45Result = list()
    with open('./trainingData.json', 'rb') as inp_fd:
        trainingData = json.load(inp_fd)
    with open('./allEdgeCounts.json', 'rb') as inp_fd:
        allEdgeCounts = json.load(inp_fd)
    for key in allEdgeCounts.keys():
        print key, allEdgeCounts.get(key)
        row = {'filename': key, 'foreheadHorizEdges': allEdgeCounts.get(key).get('foreheadEdges').get('horizontal'),
                #'foreheadVertEdges': allEdgeCounts.get(key).get('foreheadEdges').get('vertical'),
                'eyeSidesHorizEdges': allEdgeCounts.get(key).get('eyeSidesEdges').get('horizontal'),
                'eyeSidesVertEdges': allEdgeCounts.get(key).get('eyeSidesEdges').get('vertical'),
                'lipSidesHorizEdges': allEdgeCounts.get(key).get('lipSidesEdges').get('horizontal'),
                'lipSidesVertEdges': allEdgeCounts.get(key).get('lipSidesEdges').get('vertical'),
                'chinHorizEdges': allEdgeCounts.get(key).get('chinEdges').get('horizontal'),
                'chinVertEdges': allEdgeCounts.get(key).get('chinEdges').get('vertical'),
                'emotion': emotionCategories[trainingData.get(key).index(1)]
                }
        #emotion = row.pop('emotion')
        decisionTree = c45Miner.mine_c45(row, 'emotion')
        rules = c45Miner.tree_to_rules(decisionTree)
        print decisionTree
        print rules
    #    c45Result.append(row.pop('emotion'))
